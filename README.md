...
In server side, Bragi will scrape resources from multiple upstream service providers (e.g. YouTube, Spotify, NetEase Music), reformat them and expose RPC API for execute.
...
```rust
use bragi_core::scraper::{spotify::SpotifyScraper, ScraperManager};
...
async fn main() {
    ...
    manager.add_scraper(
        SpotifyScraper::try_new(
            ...
            Path::new(&static_dir).to_path_buf(),
        )
        .await
